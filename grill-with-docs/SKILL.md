...
**Documented**
- <one line per CONTEXT.md term resolved or ADR written this session, with its path>
...
Drop a heading when its list is empty. An early stop and an exhausted tree use the same marker —
the open list is what tells them apart.

The ledger is a record, not a re-litigation. One line per decision. Do not restate the reasoning.
Do not reopen a settled branch. Do not suggest a next skill. The ledger itself stays in the
terminal. The only files this session writes are `CONTEXT.md` and ADRs, written inline as
decisions land.
